# dlmBNSimMC.R
# Hierarchical simulation of BN model for MC estimation
...
#       set.seed(153)
        L<-chol(S)%x%chol(C)
        z<-rnorm(length(M))
        if(length(M)==1) return(matrix(t(L)%*%z,nrow=nrow(C),ncol=ncol(S)))
        else return(matrix(as.vector(M)+t(L)%*%z,nrow=nrow(C),ncol=ncol(S)))    
        }
...
A<-(matrix(runif(J*T),nr=T,nc=J)>0.5)*matrix(runif(J*T,max=exp(9)),nrow=T,ncol=J) # total "advertising" across all sites, for each equation 
...
for(j in 1:J) aA[,j]<-1-Cvec[j]-Uvec[j]*Ac[,j]-delta*(1-(A[,j]>0))
gA<-log(1+A)
...
for(n in 2:N) F2t<-rbind(F2t,dlm::bdiag(matrix(c(1,rep(0,J)),nc=1+J),diag(P)))
FF[[2]]<-F2t

Gt<-diag(1+J+P)
Gt[1]<-(1-delta)
Gt[1,1+1:J]<-gA[t,]
if(include.cu) for(j in 1:J) Gt[1+j,1+j]<-(1-Cvec[j]-Uvec[j]*Ac[t,j])-delta*(1-(A[t,j]>0)) else diag(Gt)[2:(1+J)]<-1

# innovation component which switches signs if 
# the underlying state variable does (simplified here)
Ht<-matrix(0,nr=nrow(Gt),nc=J)
for(j in 1:J) for(k in 1:J){
#	if(Theta2t[1+j,k] < 0) Ht[1+j,k] <- -delta*(1-(A[t,j]>0)) - Evec[j,k]*E[t,j]	# if it is below zero
#	else Ht[j+1,k] <- delta*(1-(A[t,j]>0)) + Evec[j,k]*E[t,j]	# if above
#	if(Theta2t[1+j,k] < 0) Ht[1+j,k] <- - Evec[j,k]*E[t,j]	# if it is below zero
#	else Ht[j+1,k] <- Evec[j,k]*E[t,j]	# if above
#	if(j!=k) Ht[1+j,k] <- -delta*(1-(A[t,j]>0)) - e[j]*E[t,j]	# if it is below zero
#	else Ht[j+1,k] <- delta*(1-(A[t,j]>0)) + e[j]*E[t,j]	# if above
	Ht[1+j,k] <- Evec[j,k]*E[t,j]
	}

	if(!include.H) Ht<-Ht*0
...
T1[[t]]<-Theta1t
Yt<-FF[[1]]%*%Theta1t+F12l[[t]]%*%Theta12+rmvMN(1,,V[[1]],Sigma)
...
Y<-Y[-(1:Tb),]
T1true<-list()
for(i in (Tb+1):T) T1true[[i-Tb]]<-T1[[i]]
T2true<-list()
for(i in (Tb+1):T) T2true[[i-Tb]]<-T2[[i]]
F12<-list()
for(i in (Tb+1):T) F12[[i-Tb]]<-F12l[[i]]
F1m<-list()
for(i in (Tb+1):T) F1m[[i-Tb]]<-F1ml[[i]]
F1<-list()
for(i in (Tb+1):T) F1[[i-Tb]]<-F1l[[i]]
Y<-list()
for(i in (Tb+1):T) Y[[i-Tb]]<-Yl[[i]]
...
mcmod<-list(dimensions=dimensions,Y=Y,E=El,A=Al,X=F12,F1=F1m,F2=F2)
